...
utilities such as \smtexttt{grep}, \smtexttt{awk}, and
\smtexttt{wc}.
...
simple, lexically-oriented tools such as \smtexttt{grep} or search-and-replace
...
\label{ssec-java}
   ...
   receiving the message.  For example, an instance of the \smtexttt{ColoredBall}
   class may respond to the \smtexttt{draw} message differently than an instance
   of a \smtexttt{Ball} class.  This ability to behave differently upon receipt
...
The \smtexttt{<email>} is an open tag for the \smtexttt{email}
element.  The \smtexttt{</email>} at the end of the example is the
...
\smtexttt{<\emph{tag-name}/>}.
In the above document, the \smtexttt{email}
element contains two immediate children elements: a \smtexttt{head} and a
\smtexttt{body}.  Additionally, an XML open tag can associate
attribute/value pairs with an element.  For example, the \smtexttt{body} element
above has the value \smtexttt{ascii} for its \smtexttt{encoding}
...
\smtexttt{email} element must contain exactly one \smtexttt{head}
followed by exactly one \smtexttt{body} element.  The
\smtexttt{head}, in turn, must contain \smtexttt{to} and
\smtexttt{from} elements, in that order, followed by an optional
\smtexttt{subject} element.  Each of those elements may contain text 
(also know as \emph{parsed
character data} or \smtexttt{PCDATA}).  The single \smtexttt{ATTLIST}
declaration in the DTD specifies that the \smtexttt{body} element
\emph{must} specify a value of either \smtexttt{ascii} or
\smtexttt{mime} for the \smtexttt{encoding} attribute, and
\emph{may} specify a value for the \smtexttt{encrypted} attribute.
...
\smtexttt{to} element is missing from an \smtexttt{email} document,
...
have folded all of the information contained in the \smtexttt{head}
into attributes of the \smtexttt{email} element if we chose.  There
...
could have multiple \smtexttt{to} elements contained by the
\smtexttt{head} with only a simple change to the DTD).
...
elements can then be referred to by \smtexttt{IDREF} attributes
of other elements.  A well-formed XML document must have every
\smtexttt{IDREF} value match an \smtexttt{ID} given in the document.
...
convert the \smtexttt{FirstApplet.java} implementation to:
...
exactly what the \smtexttt{stripsgml}~\cite{perlSGML} utility
...
For the \smtexttt{FirstApplet.java} source code listed above, we represent 
...
\smtexttt{literal-string} element is nested inside the
\smtexttt{send} element.  This nesting is even more apparent
...
\smtexttt{idref} attribute of that \smtexttt{var-ref} tag points
back at the referenced \smtexttt{formal-argument} element (through its
\smtexttt{id} attribute).  (The \smtexttt{id} value chosen for
...
locate because of their distinguishing \smtexttt{var-ref} tag.
Additionally, they are disambiguated from an assignment to a variable
which is converted into a \smtexttt{var-set} tag.
...
Attributes are used for modifiers such as \smtexttt{synchronized} and
\smtexttt{final} and for visibility settings such as
\smtexttt{public} or \smtexttt{private}.  Attributes are not used
...
processing on the attribute's value "\smtexttt{int[][]}" to determine that the base
type of that two-dimensional array was the primitive type
\smtexttt{int}.  Instead, types are modeled as explicit
child elements such as \smtexttt{<type name="int" dimensions="2">}.
...
For example, \smtexttt{45} and \smtexttt{1.9} are represented as:
\smtexttt{<literal-number kind="integer" value="45">} and
\smtexttt{<literal-number kind="float" value="1.9">},
respectively.  An alternate possible markup is:
\smtexttt{<literal-integer value="45">} and
\smtexttt{<literal-float value="1.9">} but using separate
...
disambiguate these literals based on a \smtexttt{kind} attribute. Thus,
we can still
...
Both \smtexttt{for} and \smtexttt{while} loops
...
\smtexttt{for-loop} and \smtexttt{while-loop}, JavaML uses a single
\smtexttt{loop} element that has a \smtexttt{kind} attribute with
value either \smtexttt{for} or \smtexttt{while}.  When a
\smtexttt{while} loop is converted, it will have neither \smtexttt{initializer} nor
\smtexttt{update} children, yet a \smtexttt{for} loop could potentially contain many
of each.  In contrast, distinct \smtexttt{do-loop} elements are used for
\smtexttt{do} loops because they have their test performed at the end of the
...
segment \smtexttt{int dx, dy;}
defines two variables both of type \smtexttt{int}, but with perhaps a
subtle additional intention: that the two variables have the same type.
For contrast, consider \smtexttt{int weight, i;}.
...
\smtexttt{continued="true"} attribute on variable declarations that
...
\smtexttt{XMLUnparse} method to each of the AST nodes. This change,
...
Toolkit~\cite{Cassowary}.
The processing of all those files takes only
about six seconds on the author's RedHat6-based dual Pentium III-450 machine.
Each of the files converted was then validated with respect to the
JavaML DTD using James Clark's Jade package's \smtexttt{nsgmls} tool~\cite{Jade}.
...
See section~\ref{sec-future} for
...
and simple APIs for working with the
...
\smtexttt{sgcount} does an excellent job of summarizing the constructs
in a Java program:
...
Notice that the output of \smtexttt{sggrep} is also a (not necessarily
...
The \smtexttt{sgrpg} (SGML RePort Generator) program permits combining
...
\smtexttt{visibility} and \smtexttt{name} attributes of the
...
\smtexttt{isBall} to \smtexttt{FIsBall} using the Perl package 
\smtexttt{XML::DOM} and the following in the
\smtexttt{start\_tag} subroutine:
...
documenting and comparing DTDs (e.g., \smtexttt{dtd2html} and
\smtexttt{dtddiff}) that would be helpful when applying this
technique.
...
well with parsing techniques~\cite{EmpAnCpp,PCP3}.
...
parser (e.g., XML4C++~\cite{XML4C++}) to construct the XML
...
queries (such as some of those described earlier in section~\ref{sec-leveraging}).
...
M.Sc. degree in 1998.  He graduated \textit{magna cum laude} with a
B.S. degree in Mathematics and Computer Science from
