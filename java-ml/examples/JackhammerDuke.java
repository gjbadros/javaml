/*
 * @(#)JackhammerDuke.java	1.3 97/07/22 Bob Weisblatt
 *
...
public class JackhammerDuke extends java.applet.Applet implements Runnable {
   ...
   public void init()
   {
	...
	imgs = getImage(getDocumentBase(), "images/jack.gif");

	if (imgs != null) {
	    x = (getSize().width - imgsWidth/nimgs) / 2;
	    while (getSize().width > 0 && getSize().height > 0 && kicker != null) {
		if (seqslot == 0) {
		    play(getDocumentBase(), "audio/jackhammer-short.au");
		}
	...
	if ((imgs != null) && (loopslot < nimgs)) {
	    ...
	    x = Math.max(0, Math.min(getSize().width - w, x + Math.random() * 4 - 2));
  ...
  public String getAppletInfo() {
    return "Title: UnderConstruction\nAuthor: Bob Weisblatt (major thievery from jag, maybe wholesale is a better word.)\nA simple java.applet.Applet class to play an image loop.";
      }
}
